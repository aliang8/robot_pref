...
def check_data_for_nans(data):
    ...
    for key, value in data.items():
        if isinstance(value, torch.Tensor):
            ...
            nan_percentage = 100 * nan_count / total_elements if total_elements > 0 else 0
            ...
            if nan_count > 0:
                print(f"  Field '{key}' contains {nan_count}/{total_elements} ({nan_percentage:.2f}%) NaN values")
...
def plot_returns_histogram(returns, output_path, bins=50, title=None):
    ...
    plt.axvline(pct_25, color='orange', linestyle='dotted', linewidth=1,
               label=f'25th percentile: {pct_25:.2f}')
    plt.axvline(pct_75, color='purple', linestyle='dotted', linewidth=1,
...
def classify_episodes_by_return(returns, num_bins=3):
    ...
    for level in range(num_bins):
        ...
        level_name = ["random", "medium", "expert"][level] if num_bins == 3 else f"level_{level}"
...
def main():
    parser = argparse.ArgumentParser(description="Create a balanced dataset with equal amounts of random/medium/expert episodes.")
    ...
    parser.add_argument("--output_dir", type=str, default="balanced_datasets",
    ...
    if args.data_path is None and args.dataset_idx is None:
        args.dataset_idx = 0
        print(f"No dataset specified. Using first dataset (index 0): {DEFAULT_DATA_PATHS[0]}")
    ...
    histogram_path = os.path.join(args.output_dir, f"{dataset_name}_returns_histogram.png")
