...
def visualize_query_and_matches(images, query_indices, match_indices_list, distances, dataset_indicators=None, output_file=None):
    ...
    query_anim = create_segment_animation(images_cpu, start_q, end_q, title="Query Segment")
    ...
    if output_file:
        create_comparison_video(
            images_cpu, query_indices, match_indices_list, distances, 
            dataset_indicators, output_file
        )
        print(f"Saved comparison video to {output_file}")

def create_comparison_video(images, query_indices, match_indices_list, distances, dataset_indicators=None, output_file="segment_comparison.mp4"):
...
def main():
    ...
    parser.add_argument("--samples_per_dataset", type=int, default=500,
                        help="Number of segments to sample from each dataset")
    parser.add_argument("--top_k", type=int, default=5,
                        help="Number of top matches to find")
    parser.add_argument("--output_dir", type=str, default="segment_matches",
    ...
    for dataset_idx, data_path in enumerate(args.data_paths):
        ...
        if args.samples_per_dataset > 0 and args.samples_per_dataset < len(segments):
            ...
            segments, segment_indices = sample_segments(segments, segment_indices, args.samples_per_dataset)
    ...
    visualize_query_and_matches(
        images, query_indices, top_match_indices, top_distances, 
        dataset_indicators, output_file=output_video
    )
