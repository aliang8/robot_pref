...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Cluster robot trajectories based on end-effector positions"
    )
    parser.add_argument(
        "--data_path",
        type=str,
    ...
    parser.add_argument(
        "--n_clusters", type=int, default=5, help="Number of clusters to extract"
    )
    parser.add_argument(
        "--max_segments",
        type=int,
    ...
    print("\n" + "=" * 50)
    print(
        f"Clustering robot trajectories based on end-effector {'relative differences' if args.use_relative_differences else 'positions'}"
    )
    print(
        f"Data path: {args.data_path if args.preprocessed_data is None else args.preprocessed_data}"
    )
    ...
    if args.preprocessed_data is not None:
        ...
        if (
            ...
            and preproc_data["use_relative_differences"]
            != args.use_relative_differences
        ):
            print(
                f"WARNING: Preprocessed data was created with use_relative_differences={preproc_data['use_relative_differences']}"
            )
            print(
                f"Current setting is use_relative_differences={args.use_relative_differences}"
            )
    ...
    distance_matrix, idx_mapping = compute_dtw_distance_matrix(
        segments, max_segments=args.max_dtw_segments
    )
    ...
    output_file = os.path.join(
        args.output_dir, f"eef_{segment_type}_dtw_distance_matrix.png"
    )
    plt.savefig(output_file, dpi=300, bbox_inches="tight")
    plt.close()
    ...
    Z = perform_hierarchical_clustering(
        distance_matrix, n_clusters=args.n_clusters, method=args.linkage_method
    )
    ...
    plot_clustering(
        ...
        output_file=os.path.join(
            args.output_dir, f"eef_{segment_type}_clustering_dendrogram.png"
        ),
    )
