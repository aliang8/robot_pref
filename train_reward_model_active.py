...
def train_final_reward_model(
    ...
    final_dataset = PreferenceDataset(
        data, labeled_pairs, segment_start_end, labeled_preferences
    )
    ...
    print(
        f"Using all {data_loaders['train_size']} labeled samples for training the final model (no validation split)"
    )
...
def active_preference_learning(cfg, dataset_name=None):
    ...
    print(f"  Maximum queries: {cfg.active_learning.total_queries}")
    print(
        f"  Number of queries per iteration: {cfg.active_learning.total_queries_per_iteration}"
    )
    print(
        f"  Fine-tuning: {'Enabled' if cfg.active_learning.fine_tune else 'Disabled'}"
    )
    ...
    episodes = process_data_trajectories(data)
    ...
    if dtw_enabled:
        ...
        dtw_matrix_file = (
            Path(cfg.data.data_path).parent
            / f"dtw_matrix_{cfg.data.segment_length}.pkl"
        )

        if not os.path.exists(dtw_matrix_file):
            print(f"Warning: DTW matrix file not found at {dtw_matrix_file}")
            assert False, (
                "DTW matrix file not found. Please run preprocess_dtw_matrix.py"
            )
        else:
            distance_matrix, segment_start_end = pickle.load(
                open(dtw_matrix_file, "rb")
            )
            print(
                f"Successfully loaded DTW distance matrix with shape: {distance_matrix.shape}"
            )
    else:
    ...
    test_dataset = PreferenceDataset(
        data, test_pairs, segment_start_end, test_preferences
    )
    ...
    if dtw_enabled and distance_matrix is not None:
        ...
        if len(dtw_candidate_pairs) > 0:
            print(
                f"Using {len(dtw_candidate_pairs)} pairs from DTW matrix for uncertainty sampling"
            )
            candidate_pairs = dtw_candidate_pairs
        else:
            print("Warning: No pairs found in DTW matrix. Using all unlabeled pairs.")

    while total_labeled < total_queries and len(unlabeled_pairs) > 0:
        iteration += 1
        print("\n" + "=" * 80)
        print(f"ACTIVE LEARNING ITERATION {iteration}")
        print("=" * 80)
        print(
            f"Progress: {total_labeled}/{total_queries} labeled pairs ({total_labeled / total_queries * 100:.1f}%)"
        )
        ...
        selected_query_pref = get_gt_preferences(
            data, segment_start_end, selected_query_pair
        )
        ...
        if dtw_enabled and distance_matrix is not None and len(selected_query_pair) > 0:
            print(
                f"\tAugmenting {len(selected_query_pair)} selected pairs using DTW (k={dtw_k_augment})..."
            )
            ...
            for qp, preference_val in zip(selected_query_pair, selected_query_pref):
                ...
                if preference_val == 1:
                    similar_to_dtw_i_indices = find_similar_segments_dtw(
                        dtw_i, dtw_k_augment, distance_matrix
                    )
                    ...
                    similar_to_dtw_j_indices = find_similar_segments_dtw(
                        dtw_j, dtw_k_augment, distance_matrix
                    )
            ...
            augmented_pref_with_rewards = get_gt_preferences(
                data, segment_start_end, dtw_augmented_pairs
            )
            augmented_acc = (
                np.array(augmented_pref_with_rewards)
                == np.array(dtw_augmented_preferences)
            ).mean()
            ...
            print(
                f"\tNumber of labeled pairs after DTW augmentation: {len(labeled_pairs)}"
            )
        ...
        val_loader = data_loaders["val"]
        print(
            f"Using all {data_loaders['train_size']} labeled samples for training (no validation split)"
        )
        ...
        if cfg.active_learning.fine_tune and prev_ensemble is not None:
            ...
            if len(ensemble.models) != cfg.active_learning.num_models:
                print(
                    f"Warning: Previous ensemble has {len(ensemble.models)} models, but {cfg.active_learning.num_models} requested. "
            ...
            lr = (
                cfg.active_learning.fine_tune_lr
        ...
        ensemble, train_loss, val_loss = train_model(
            ...
            output_path=os.path.join(
                model_dir, "rm_training", f"reward_model_training_{iteration}.png"
            ),
        )
        ...
        test_metrics = evaluate_model_on_test_set(
            ensemble.models[0], test_loader, device
        )
            ...
            log_to_wandb(
                {
                    ...
                    if metrics["val_losses"][-1] is not None
                    else 0,
                },
                wandb_run=wandb_run,
            )
        ...
        batch_size = min(
            cfg.active_learning.total_queries_per_iteration,
        ...
        if iteration % cfg.training.reward_analysis_every == 0:
            ...
            analyze_rewards(
                ...
                output_file=os.path.join(
                    model_dir, "reward_analysis", f"reward_grid_iter_{iteration}.png"
                ),
                num_episodes=9,
                wandb_run=wandb_run,
    ...
    if test_loader is not None:
        consistent_metrics = evaluate_model_on_test_set(
            final_model, test_loader, device
        )
        print(
            f"Final model accuracy on consistent test set: {consistent_metrics['test_accuracy']:.4f}"
        )
    else:
