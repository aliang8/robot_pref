...
def create_segments(embeddings, episode_ids, H):
    ...
    print(
        f"Creating segments with window size H={H} from embeddings of shape {embeddings.shape}"
    )
    segments = []
    ...
    for episode_id in tqdm(unique_episodes, desc="Processing episodes"):
        ...
        for i in range(0, len(ep_embeddings) - H + 1):
            ...
            segment_indices.append(
                (int(ep_indices[i].item()), int(ep_indices[i + H - 1].item()))
            )

        if num_segments_in_episode == 0:
            print(
                f"Episode {episode_id} has {len(ep_embeddings)} frames (less than H={H}), skipping"
            )
...
def perform_hierarchical_clustering(distance_matrix, n_clusters=5):
    ...
    if np.any(non_finite_mask):
        ...
        print(
            f"WARNING: Found {non_finite_count} non-finite values in distance matrix!"
        )
    ...
    if not np.all(np.isfinite(condensed_dist)):
        raise ValueError(
            "Condensed distance matrix contains non-finite values after replacement!"
        )

    print(f"Condensed distance matrix shape: {condensed_dist.shape}")
    print(
        f"Condensed distance matrix range: [{np.min(condensed_dist):.2f}, {np.max(condensed_dist):.2f}]"
    )
...
def get_clusters(Z, n_clusters=5):
    ...
    for i, (cluster, count) in enumerate(zip(unique_clusters, counts)):
        print(
            f"  Cluster {cluster}: {count} segments ({count / len(clusters) * 100:.1f}%)"
        )

    return clusters


def sample_segment_pairs_from_clusters(
    segments, segment_indices, cluster_assignments, rewards, n_pairs=10
):
    ...
    for i, ((idx1, idx2), (c1, c2)) in enumerate(zip(pairs, cluster_combinations)):
        print(
            f"  Pair {i + 1}: Segment {idx1} (Cluster {c1}) vs Segment {idx2} (Cluster {c2})"
        )
...
def display_preference_query(
    images, segment_pairs, segment_indices, preference_labels=None
):
    ...
    for i, (idx1, idx2) in enumerate(segment_pairs):
        ...
        if preference_labels is not None:
            ...
            print(
                f"Pair {i + 1}/{len(segment_pairs)}: Synthetic preference: {'Segment 1' if pref == 1 else 'Segment 2'}"
            )
        else:
        ...
        if preference_labels is None:
            ...
            while True:
                pref = input(
                    "Enter 1 for Segment 1, 2 for Segment 2, or 0 for Equal/Can't decide: "
                )
                if pref in ["0", "1", "2"]:
                    break
...
def collect_human_preferences(
    data, segments, segment_indices, cluster_assignments, n_pairs=10
):
    ...
    display_preference_query(
        data["image"], segment_pairs, segment_indices, synthetic_prefs
    )

    # Collect human preferences
    print("\nNow collecting your preferences:")
    human_prefs = []
    pairs_with_prefs = display_preference_query(
        data["image"], segment_pairs, segment_indices
    )
...
def create_preference_video(
    ...
    segment_indices,
    preference_labels,
    output_file="preference_video.mp4",
    num_pairs=3,
):
    ...
    for i in range(num_pairs):
        ...
        pref_text = ax_info.text(
            0.5,
            0.5,
            f"Preferred: {'Traj 1' if pref == 1 else 'Traj 2'}",
            fontsize=12,
            ha="center",
            va="center",
...
def main():
    ...
    segments_per_dataset = (
        500  # Maximum number of segments to extract from each dataset
    )
    ...
    for dataset_idx, data_path in enumerate(data_paths):
        ...
        if segments_per_dataset > 0:
            print(
                f"Sampling episodes to get approximately {segments_per_dataset} segments..."
            )
            ...
            # Subsample the data
            print(
                f"Selected {len(selected_indices)} frames from {len(unique_episodes)} episodes, expected to yield ~{estimated_segments} segments"
            )
            ...
            ep_mapping = {
                old_ep.item(): i + (dataset_idx * 10000)
                for i, old_ep in enumerate(unique_eps)
            }
            new_episode_ids = torch.tensor(
                [ep_mapping[ep.item()] for ep in episode_ids]
            )
    ...
    distance_matrix, idx_mapping = compute_dtw_distance_matrix(
        segments, max_segments=max_segments_for_dtw
    )
    ...
    if max_segments_for_dtw < len(segments):
        ...
        print(
            f"Cluster assignment coverage: {np.sum(full_cluster_assignments != -1)}/{len(segments)} segments"
        )

        # For segments that weren't clustered, assign to nearest clustered segment
        if np.any(full_cluster_assignments == -1):
            print(
                "Assigning unclustered segments to nearest clustered segment (optimized)..."
            )
            unclustered_indices = np.where(full_cluster_assignments == -1)[0]
            clustered_indices = np.where(full_cluster_assignments != -1)[0]

            print(
                f"Need to assign {len(unclustered_indices)} unclustered segments using {len(clustered_indices)} reference segments"
            )

            # Convert segments to numpy arrays for faster processing
            unclustered_segments = np.array(
                [segments[i].mean(0).numpy() for i in unclustered_indices]
            )
            clustered_segments = np.array(
                [segments[i].mean(0).numpy() for i in clustered_indices]
            )
            ...
            for batch_idx in tqdm(
                range(num_batches), desc="Assigning unclustered segments"
            ):
                ...
                differences = (
                    batch_unclustered[:, np.newaxis, :]
                    - clustered_segments[np.newaxis, :, :]
                )
                ...
                full_cluster_assignments[batch_unclustered_indices] = (
                    batch_cluster_assignments
                )

            print(
                f"All {len(unclustered_indices)} unclustered segments assigned to nearest clusters"
            )
    ...
    video_file = create_preference_video(
        ...
        segment_indices,
        synthetic_prefs,
        output_file="preference_comparison.mp4",
        num_pairs=3,
    )
