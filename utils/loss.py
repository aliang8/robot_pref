...
def bradley_terry_loss(
    rewards1, rewards2, preferences, cost=None, alpha=1.0, beta_max=1.0, eps=1e-6
):
    """
    ...
    logits = torch.clamp(beta * (rewards1 - rewards2), min=-50.0, max=50.0)
...
def distributional_reward_loss(
    ...
    reg_loss = regularization_loss(
        torch.cat([reward_vars1, reward_vars2], dim=0), eta=eta
    )
...
def arm_gripper_loss(
    action_preds, actions, arm_loss_fn, gripper_loss_fn, gaussian_output=False
):
    """
    ...
    gripper_preds_binary = torch.where(
        gripper_preds > 0.0,  # >0 -> close (1)
