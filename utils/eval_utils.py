...
def evaluate_episode_worker(worker_args):
    ...
    if record_video and video_path:
        try:
            ...
            try:
                # Create the recorder
                video_recorder = SimpleVideoRecorder(env, episode_video_path, fps=video_fps)
            except Exception as e:
...
def evaluate_policy_manual(env, algo, n_episodes=10, verbose=True, record_video=False, video_path=None, video_fps=30, parallel=True, num_workers=None):
    ...
    if use_parallel:
        try:
            ...
            print("Warning: multiprocessing not available. Falling back to sequential evaluation.")
            use_parallel = False
        except Exception as e:
    ...
    if not use_parallel:
        ...
        for episode in tqdm(range(n_episodes), desc="Evaluating policy", disable=not verbose):
            ...
            if record_this_episode:
                ...
                video_recorders[episode] = create_video_recorder(episode_env, video_path, episode, fps=video_fps)
