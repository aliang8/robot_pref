...
from trajectory_utils import RANDOM_SEED
...
def evaluate(model_path, task_name, num_episodes=20, render=False, record=False):
    ...
    for ep in range(num_episodes):
        ...
        print(f"Episode {ep+1}/{num_episodes}: Return = {ep_return:.2f}, Success = {success}, Steps = {steps}")
...
def main():
    ...
    parser.add_argument("--model_path", type=str, required=True,
                        help="Path to the trained IQL model")
    ...
    parser.add_argument("--record", action="store_true",
                        help="Record a video of the policy")
    ...
    cumulative_success = np.cumsum(results['success_rates']) / np.arange(1, args.num_episodes + 1)
