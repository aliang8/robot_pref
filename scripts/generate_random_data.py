...
def main():
    ...
    video_path = Path("/scr/matthewh6/robot_pref/mixed_data_vids") / name.replace(
        ".hdf5", "_videos"
    )
    ...
    with h5py.File(expert_data_path, "r") as f_in, h5py.File(output_path, "w") as f_out:
        ...
        if len(all_demo_keys) >= expert_trajs:
            expert_demo_keys = np.random.choice(
                all_demo_keys, expert_trajs, replace=False
            )
        else:
            expert_demo_keys = np.random.choice(
                all_demo_keys, expert_trajs, replace=True
            )
            print(
                f"Warning: Only {len(all_demo_keys)} demos available, sampling with replacement"
            )
        ...
        for demo_key in expert_demo_keys:
            ...
            print(
                f"Creating video for expert demo {demo_key_num + 1}/{len(expert_demo_keys)} from {demo_key} with {actions.shape[0]} timesteps"
            )
        ...
        for i in range(suboptimal_trajs):
            ...
            print(
                f"Generating suboptimal demo {i + 1}/{suboptimal_trajs} from {sampled_demo_key} with {actions.shape[0]} timesteps"
            )
            ...
            collected_obs = {
                k: np.stack([obs[k] for obs in collected_obs]) for k in collected_obs[0]
            }  # {key: (T, ...shape...)}
        ...
        print(
            f"Data augmentation complete! Generated {expert_trajs} expert + {suboptimal_trajs} suboptimal = {expert_trajs + suboptimal_trajs} total trajectories"
        )
