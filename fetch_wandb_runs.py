...
def plot_algorithm_comparisons(df, output_dir="algorithm_plots"):
    ...
    print(
        f"Using {len(df_filtered)} runs with valid metrics out of {len(df)} total runs"
    )
    ...
    metrics_to_plot = [
        ...
        {
            "column": "top3_avg_eval_success_rate",
            "title": "Average of Top 3 Checkpoints Success Rate",
        },
    ]
            ...
            for metric in metrics:
                if metric in df.columns:
                    ...
                    if len(values) > 0:
                        ...
                        std_val = values.std()
                        row[f"{alg}_{metric}"] = (
                            f"{mean_val:.3f}±{std_val:.3f} (n={len(values)})"
                        )
                    else:
                        row[f"{alg}_{metric}"] = "N/A"
...
def plot_reward_model_comparisons(df, output_dir="reward_model_plots"):
    ...
    if "reward_model" in df_filtered.columns:
        ...
        df_filtered["reward_model_name"] = df_filtered["reward_model"].apply(
            extract_reward_model_name
        )
    ...
    print(
        f"Using {len(df_filtered)} runs with valid metrics for reward model comparison plots"
    )
    ...
    for dataset in datasets:
        for algorithm in algorithms:
            ...
            print(
                f"Found {len(query_counts)} unique query counts for {algorithm} on {dataset}"
            )
            ...
            for num_queries in query_counts:
                ...
                if len(reward_models) < 1:
                    print(
                        f"Skipping {algorithm} on {dataset} with {query_label} - no reward model: {reward_models}"
                    )
                    continue

                print(
                    f"Creating plots for {algorithm} on {dataset} with {query_label} ({len(reward_models)} reward models)"
                )
                ...
                for metric in metrics_to_plot:
                    ...
                    # Add 95% confidence interval
                    stats["ci_95"] = (
                        1.96 * stats["std"] / np.sqrt(stats["count"].clip(1))
                    )
                    ...
                    if (
                        len(dataset_bc_df) > 0
                        and metric_column in dataset_bc_df.columns
                    ):
...
def parse_args():
    ...
    parser.add_argument(
        "--project", type=str, default="robot_pref", help="wandb project name"
    )
    parser.add_argument("--entity", type=str, default="clvr", help="wandb entity name")
    parser.add_argument(
        "--user", type=str, default="aliangdw", help="Filter for runs by this user"
    )
    parser.add_argument(
        "--max_runs", type=int, default=None, help="Maximum number of runs to fetch"
    )
    parser.add_argument(
        "--output_dir",
        type=str,
    ...
    parser.add_argument(
        "--filter", type=str, default=None, help="JSON string with additional filters"
    )
