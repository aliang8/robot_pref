...
def train(config: DictConfig):
    ...
    if "metaworld" in config.env:
        ...
        dataset = utils_env.Robomimic_dataset(
            config.data_path, return_images=True
        )  # no goals states for reward
        cross_dataset = (
            utils_env.Robomimic_dataset(config.cross_data_path, return_images=True)
            if config.cross_data_path
            else None
        )
    else:
    ...
    if config.normalize:
        ...
        dataset["goal_points"] = (
            dataset["goal_points"] - goal_points_mean
        ) / goal_points_std
        ...
        if cross_dataset is not None:
            ...
            cross_dataset["observations"] = (
                cross_dataset["observations"] - state_mean
            ) / state_std
            cross_dataset["next_observations"] = (
                cross_dataset["next_observations"] - state_mean
            ) / state_std
            ...
            cross_dataset["goal_points"] = (
                cross_dataset["goal_points"] - goal_points_mean
            ) / goal_points_std
    ...
    if config.single_emb:
        print(
            f"Using ground truth preferences for {config.data_path} for reward learning"
        )
        ...
        train_idx_1 = [
            [j for j in range(i, i + config.segment_size)] for i in train_idx_st_1
        ]
        train_idx_2 = [
            [j for j in range(i, i + config.segment_size)] for i in train_idx_st_2
        ]

        val_idx_1 = [
            [j for j in range(i, i + config.segment_size)] for i in val_idx_st_1
        ]
        val_idx_2 = [
            [j for j in range(i, i + config.segment_size)] for i in val_idx_st_2
        ]

        # Get observations and actions for segments
        train_obs_act_1 = np.concatenate(
            (dataset["observations"][train_idx_1], dataset["actions"][train_idx_1]),
            axis=-1,
        )
        train_obs_act_2 = np.concatenate(
            (dataset["observations"][train_idx_2], dataset["actions"][train_idx_2]),
            axis=-1,
        )
    ...
    #     reward_model.train_model()
        ...
        aug_idx_1 = [
            [j for j in range(i, i + config.segment_size)] for i in cross_idx_st_1
        ]
        aug_idx_2 = [
            [j for j in range(i, i + config.segment_size)] for i in cross_idx_st_2
        ]

        # Get observations and actions for augmented segments
        aug_obs_act_1 = np.concatenate(
            (
                cross_dataset["observations"][aug_idx_1],
                cross_dataset["actions"][aug_idx_1],
            ),
            axis=-1,
        )
        aug_obs_act_2 = np.concatenate(
            (
                cross_dataset["observations"][aug_idx_2],
                cross_dataset["actions"][aug_idx_2],
            ),
            axis=-1,
        )

        train_images1 = (
            cross_dataset["images"][aug_idx_1] if "images" in cross_dataset else None
        )
        train_images2 = (
            cross_dataset["images"][aug_idx_2] if "images" in cross_dataset else None
        )
