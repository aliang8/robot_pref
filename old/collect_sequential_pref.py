...
def train_reward_model_from_preferences(
    ...
    """
    print(
        f"Training {'ensemble' if use_ensemble else 'single'} reward model on {len(preferences)} preferences..."
    )
    ...
    dataset = PreferenceDataset(
        data_cpu, segment_pairs, segment_indices, preference_labels
    )
    ...
    if use_ensemble:
        ...
        model = EnsembleRewardModel(
            state_dim, action_dim, hidden_dims, num_models=num_models
        )
...
def collect_sequential_preferences(
    data,
    segments,
    segment_indices,
    n_queries=100,
    ...
    n_segments = len(segments)
    print(
        f"Collecting {n_queries} sequential preferences with k={k_augment} augmentations each..."
    )
    ...
    if active_learning_enabled:
        ...
        num_models = (
            cfg.active_learning.num_models
        ...
        if reward_model_path:
            print(
                f"Loading initial reward model from {reward_model_path} for active selection"
            )

            # Load the reward model based on uncertainty method
            if use_ensemble:
                reward_model = EnsembleRewardModel(
                    state_dim, action_dim, num_models=num_models
                )
                # TODO: Implement proper loading of ensemble model if needed
                print(
                    "Warning: Loading ensemble models from file not implemented. Initializing new ensemble."
                )
            else:
            ...
            print(
                f"No reward model provided. Will train from scratch after collecting initial preferences."
            )
    ...
    initial_random_queries = (
        min(initial_size, n_queries) if active_learning_enabled else 0
    )
    ...
    with tqdm(total=n_queries, desc="Collecting preferences") as pbar:
        while len(preferences) < n_queries:
            if not remaining_pairs:
                ...
                print(
                    f"Total compared pairs: {len(compared_pairs)}/{n_segments * (n_segments - 1) // 2}"
                )
            ...
            use_active = (
                ...
                and len(preferences) >= initial_random_queries
            ...
            if use_active:
                ...
                n_candidates = min(
                    cfg.active_learning.n_candidates, len(remaining_pairs)
                )
            ...
            compared_pairs.add(
                (j, i)
            )  # Also add the reverse pair to avoid both orderings
            ...
            if k_augment > 0:
                ...
                if pref == 1:
                    ...
                    similar_to_i = find_similar_segments(
                        segments, i, k=k_augment, distance_matrix=distance_matrix
                    )
                    ...
                    similar_to_j = find_similar_segments(
                        segments, j, k=k_augment, distance_matrix=distance_matrix
                    )
...
def verify_augmented_preferences(
    data, segment_indices, preferences, augmented_preferences
):
    ...
    print(
        f"Augmentation accuracy: {stats['accuracy']:.2%} ({stats['correct']}/{comparable} comparable pairs)"
    )
...
def create_preference_dataset(data, segment_indices, preferences, output_file):
    ...
    print(
        f"Saving preference dataset to {output_file} with {len(preference_labels)} preferences"
    )
    torch.save(preference_dataset, output_file)
    print(
        f"Successfully saved preference dataset with included fields: {list(compact_data.keys())}"
    )
...
def create_grid_video(images, segments_info, output_file, title=None, grid_size=None):
    ...
    for i, (segment, seq) in enumerate(zip(segments_info, sequences)):
        ...
        if border_color is None:
            border_color = (
                preferred_color if segment["is_preferred"] else non_preferred_color
            )
    ...
    anim = animation.FuncAnimation(
        fig, update, frames=max_length, interval=100, blit=True
    )
...
def create_augmented_grid_video(
    images,
    original_pair,
    augmented_pairs,
    segment_indices,
    ...
    print(
        f"Creating grid video with {n_rows} rows (1 original + {len(augmented_pairs)} augmentations)"
    )
    ...
    gt_pref = None
    if "reward" in data:
        gt_pref = get_reward_based_preference(
            data, segment_indices[i], segment_indices[j]
        )
    ...
    for aug_idx, (aug_i, aug_j, aug_pref) in enumerate(augmented_pairs):
        ...
        aug_gt_label = ""
        if "reward" in data:
            aug_gt_pref = get_reward_based_preference(
                data, segment_indices[aug_i], segment_indices[aug_j]
            )
            if aug_gt_pref == 1:
...
def visualize_all_augmentations(
    data,
    segments,
    segment_indices,
    direct_preferences,
    ...
    if "image" not in data:
        print(
            "WARNING: No image data found in dataset. Cannot create grid visualizations."
        )
        return []
    ...
    if similar_segments_info:
        ...
        if len(similar_segments_info) > max_visualizations:
            ...
            selected_indices = random.sample(
                range(len(similar_segments_info)), max_visualizations
            )
        else:
        ...
        for aug_idx, (aug_i, aug_j, aug_pref) in enumerate(augmented_preferences):
            ...
            if not found_match:
                print(
                    f"DEBUG: Augmentation {aug_idx} ({aug_i}, {aug_j}) not matched to any direct preference"
                )
        ...
        if len(augmentation_groups) > max_visualizations:
            selected_indices = random.sample(
                list(augmentation_groups.keys()), max_visualizations
            )
        else:
            selected_indices = list(augmentation_groups.keys())

    print(
        f"Creating visualizations for {len(selected_indices)} randomly selected preference pairs"
    )
    ...
    for idx in selected_indices:
        ...
        print(
            f"DEBUG: Creating visualization for preference {idx} with {len(augmented_pairs)} augmentations"
        )
...
@hydra.main(
    config_path="config", config_name="sequential_preferences", version_base=None
)
def main(cfg: DictConfig):
    ...
    # Setup device
    device = None
    if hasattr(cfg, "active_learning") and getattr(
        cfg.active_learning, "enabled", False
    ):
        if cfg.hardware.use_cpu:
            device = torch.device("cpu")
        else:
            device = torch.device(
                f"cuda:{cfg.hardware.gpu}" if torch.cuda.is_available() else "cpu"
            )
    ...
    if hasattr(cfg, "active_learning"):
        ...
        uncertainty_method = getattr(
            cfg.active_learning, "uncertainty_method", "entropy"
        )
    ...
    if use_dtw_distance and distance_matrix is None:
        ...
        distance_matrix, idx_mapping = compute_dtw_distance_matrix(
            segments, max_segments=max_dtw_segments, random_seed=random_seed
        )
        ...
        if max_dtw_segments is not None and max_dtw_segments < len(segments):
            ...
            segment_indices = [
                segment_indices[idx_mapping[i]] for i in range(len(idx_mapping))
            ]

    # Collect sequential preferences with augmentation
    all_preferences, distance_matrix, similar_segments_info = (
        collect_sequential_preferences(
            data,
            segments,
            segment_indices,
            n_queries=n_queries,
    ...
    if use_ground_truth and "reward" in data:
        verification_stats = verify_augmented_preferences(
            data, segment_indices, direct_preferences, augmented_preferences
        )
    else:
    ...
    preference_dataset = create_preference_dataset(
        data, segment_indices, all_preferences, dataset_file
    )
