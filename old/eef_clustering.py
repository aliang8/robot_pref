...
def extract_eef_trajectories(
    data, segment_length=20, max_segments=None, use_relative_differences=False
):
    ...
    if use_relative_differences:
        print(
            f"Creating relative difference segments with window size H={segment_length}"
        )
        # Need segment_length + 1 for computing differences
        effective_length = segment_length + 1
    else:
        print(
            f"Creating absolute position segments with window size H={segment_length}"
        )
    ...
    with tqdm(total=max_segments, desc="Sampling segments") as pbar:
        ...
        while len(segments) < max_segments and total_attempts < max_attempts:
            ...
            if (
                total_attempts >= 10 * max_segments
                and len(segments) < max_segments / 10
            ):
                print(
                    f"Warning: Made {total_attempts} attempts but only found {len(segments)} valid segments. Stopping early."
                )
                break

    print(
        f"Created {len(segments)} {'difference' if use_relative_differences else 'absolute position'} segments with {total_attempts} sampling attempts"
    )
    if segments:
...
def plot_representative_trajectories(
    ...
    for i, cluster_id in enumerate(unique_clusters):
        ...
        cluster_segments = [
            segments[j] for j in range(len(segments)) if clusters[j] == cluster_id
        ]
        ...
        for j in range(n_samples):
            ...
            ax.scatter(
                traj[0, 0], traj[0, 1], traj[0, 2], color="green", s=50, label="Start"
            )
            ax.scatter(
                traj[-1, 0], traj[-1, 1], traj[-1, 2], color="red", s=50, label="End"
            )
        ...
        plt.tight_layout()
        plt.savefig(
            f"{output_dir}/cluster_{cluster_id}_representative.png",
            dpi=300,
            bbox_inches="tight",
        )
        plt.close()
...
def create_combined_grid_video(
    data,
    segments,
    original_segments,
    segment_indices,
    clusters,
    n_per_cluster=5,
    output_file="combined_grid_video.mp4",
    global_ranges=None,
):
    ...
    print(
        f"Creating combined grid with {n_clusters} clusters, {n_per_cluster} segments per cluster"
    )
    ...
    def update(frame):
        ...
        if has_image_data:
            for i, (img_plot, segment) in enumerate(
                zip(all_image_plots, all_segment_frames)
            ):
...
def main(cfg: DictConfig):
    ...
    if preprocessed_data is not None:
        ...
        from utils.trajectory import load_preprocessed_segments
        ...
        if (
            "use_relative_differences" in preproc_data
            and preproc_data["use_relative_differences"] != use_relative_differences
        ):
            print(
                f"WARNING: Preprocessed data was created with use_relative_differences={preproc_data['use_relative_differences']}"
            )
            print(
                f"Current setting is use_relative_differences={use_relative_differences}"
            )
        ...
        if not skip_videos:
            ...
            if not has_essential_data:
                print(
                    f"Preprocessed data doesn't contain necessary observation data for visualization."
                )
    ...
    if max_dtw_segments is not None and max_dtw_segments < len(segments):
        segments_for_clustering = [
            segments[idx_mapping[i]] for i in range(len(idx_mapping))
        ]
        original_segments_for_clustering = [
            original_segments[idx_mapping[i]] for i in range(len(idx_mapping))
        ]
    ...
    output_file = os.path.join(
        output_dir, f"eef_{segment_type}_dtw_distance_matrix.png"
    )
    plt.savefig(output_file, dpi=300, bbox_inches="tight")
    plt.close()
    ...
    Z = perform_hierarchical_clustering(
        distance_matrix, n_clusters=n_clusters, method=linkage_method
    )
    ...
    plot_clustering(
        ...
        output_file=os.path.join(
            output_dir, f"eef_{segment_type}_clustering_dendrogram.png"
        ),
    )
