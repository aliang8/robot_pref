...
def load_data(processed_data_path="processed_data.pkl", preference_data_path="preference_data.pkl"):
...
def plot_dataset_distribution_by_cluster(cluster_assignments, dataset_indicators, n_clusters, data_paths):
    """Plot the distribution of datasets across clusters."""
    # Create a more readable label for each dataset
    dataset_labels = [f"DS{i+1}: {os.path.basename(path)}" for i, path in enumerate(data_paths)]
    ...
    if len(dataset_indicators) != len(cluster_assignments):
        print(f"Warning: dataset_indicators length ({len(dataset_indicators)}) doesn't match cluster_assignments length ({len(cluster_assignments)})")
    ...
    for cluster in range(1, n_clusters+1):
        ...
        cluster_dataset_counts[cluster] = [dataset_counts.get(d, 0) for d in unique_datasets]
    ...
    print(f"Dataset distribution plots saved to dataset_distribution_by_cluster.png and dataset_counts_by_cluster.png")

def plot_cluster_distribution_by_dataset(cluster_assignments, dataset_indicators, n_clusters, data_paths):
    ...
    fig, axes = plt.subplots(len(unique_datasets), 1, figsize=(12, 5*len(unique_datasets)))
    ...
    for i, dataset_idx in enumerate(unique_datasets):
        ...
        if len(dataset_indicators) != len(cluster_assignments):
            ...
            dataset_mask = np.array([segment_to_dataset.get(i, -1) == dataset_idx for i in range(len(cluster_assignments))])
        else:
    ...
    plt.close()
    print(f"Cluster distribution by dataset plot saved to cluster_distribution_by_dataset.png")

def plot_reward_by_cluster(cluster_assignments, segment_indices, rewards, dataset_indicators=None):
    ...
    if dataset_indicators is not None:
        ...
        cluster_dataset_rewards = {c: {d: [] for d in unique_datasets} for c in unique_clusters}
        ...
        for cluster in sorted(unique_clusters):
            ...
            # Prepare data for boxplot
            data = [cluster_dataset_rewards[cluster][d] for d in sorted(unique_datasets) if len(cluster_dataset_rewards[cluster][d]) > 0]
            labels = [f'Dataset {d+1}' for d in sorted(unique_datasets) if len(cluster_dataset_rewards[cluster][d]) > 0]
...
def create_cluster_summary_video(images, cluster_assignments, segment_indices, dataset_indicators=None, output_file='cluster_summary.mp4'):
    ...
    for i, cluster in enumerate(unique_clusters):
        ...
        cluster_title = fig.text(0.02, 1 - (i + 0.5) / n_rows, f"Cluster {cluster}", fontsize=14, ha='left', va='center')
        ...
        for j, (exemplar, dataset_idx) in enumerate(zip(cluster_exemplars, cluster_datasets)):
...
def analyze_preferences(segment_pairs, segment_indices, synthetic_prefs, rewards):
    ...
    print(f"Synthetic preference accuracy: {correct_pref_count}/{len(pair_rewards)} " 
    ...
    reward_diffs = [r1 - r2 if pref == 1 else r2 - r1 
                    for (r1, r2, pref) in pair_rewards]
...
def main():
    ...
    parser.add_argument('--preference-data', type=str, default="preference_data.pkl",
                        help='Path to preference data pickle file')
    parser.add_argument('--create-videos', action='store_true',
                        help='Create videos (may be slow)')
    parser.add_argument('--tsne', action='store_true',
                        help='Create t-SNE visualization (may be slow)')
    ...
    # Load data
    processed_data, preference_data = load_data(args.processed_data, args.preference_data)
    ...
    if dataset_indicators is not None:
        ...
        for dataset_idx, count in sorted(dataset_counts.items()):
            print(f"  Dataset {dataset_idx+1}: {count} samples " + 
                  (f"({data_paths[dataset_idx]})" if dataset_idx < len(data_paths) else ""))
    ...
    if dataset_indicators is not None and data_paths:
        ...
        plot_dataset_distribution_by_cluster(cluster_assignments, dataset_indicators, n_clusters, data_paths)
        plot_cluster_distribution_by_dataset(cluster_assignments, dataset_indicators, n_clusters, data_paths)
    ...
    if original_data is not None and 'reward' in original_data:
        analyze_preferences(segment_pairs, segment_indices, synthetic_prefs, original_data['reward'])
