...
class SkillEncoder(nn.Module):
    ...
    def forward(self, obs, actions):
        ...
        x = torch.cat(
            [obs, actions], dim=-1
        )  # [batch_size, seq_len, obs_dim + action_dim]
...
class SkillDecoder(nn.Module):
    ...
    def forward(self, obs, z):
        ...
        x = torch.cat(
            [obs, z_expanded], dim=-1
        )  # [batch_size, seq_len, obs_dim + latent_dim]
...
class SkillVAE(nn.Module):
    ...
    def __init__(
        self, obs_dim, action_dim, latent_dim=32, hidden_dim=256, num_layers=2
    ):
        ...
        self.encoder = SkillEncoder(
            obs_dim, action_dim, latent_dim, hidden_dim, num_layers
        )
        self.decoder = SkillDecoder(
            obs_dim, action_dim, latent_dim, hidden_dim, num_layers
        )
    ...
    def compute_vae_loss(self, obs, actions, beta=1.0):
        ...
        recon_loss = F.mse_loss(
            outputs["reconstructed_actions"], actions, reduction="mean"
        )

        # KL divergence between posterior and prior
        posterior = torch.distributions.Normal(
            outputs["z_mean"], torch.exp(0.5 * outputs["z_logvar"])
        )
        prior = torch.distributions.Normal(
            outputs["prior_mean"], torch.exp(0.5 * outputs["prior_logvar"])
        )
    ...
    def compute_total_loss(
        self, obs1, actions1, obs2, actions2, time_diff, beta=1.0, alpha=1.0
    ):
        """
